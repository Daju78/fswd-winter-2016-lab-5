...
describe('todo', function() {
  describe('with no existing todos', function() {
    describe('API calls', function() {
      ...
      //     .post('/todo/new')
      //     .set('Accept', 'application/json')
      //     .send({ todo: 'My new todo' })
      //     .expect(302)
      //     .expect('Location', '/todo');
      // });
      it('should return the details of the new task', function() {
        return request(app)
          .post('/todo/new')
          .set('Accept', 'application/json')
          .send({ todo: 'My new todo' })
          .expect(200)
          .then(function(res) {
            res.body.should.contain.all.keys('id', 'UserId', 'createdAt', 'updatedAt', 'completedAt');
            res.body.title.should.equal('My new todo');
          });
      })
    });
